...
export class UsersService {
  ...
  async findAll(
    requestingUser: Pick<AuthenticatedUser, 'organizationId' | 'isSuperAdmin'>
  ): Promise<User[]> {
    ...
    return this.userRepo
      .createQueryBuilder('u')
      .innerJoin('user_roles', 'ur', 'ur.user_id = u.id AND ur.organization_id = :organizationId', {
  ...
  async update(
    ...
    profile.name = dto.firstName && dto.lastName ? `${dto.firstName} ${dto.lastName}` : profile.name
