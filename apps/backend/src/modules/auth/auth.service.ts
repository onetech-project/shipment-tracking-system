...
export class AuthService {
  ...
  async refreshToken(
    ...
    return this.issueTokenPair(user, tokenRecord.organizationId, tokenRecord.familyId, ip, userAgent);
  }
  ...
  private async issueTokenPair(
    ...
    const expiresAt = new Date(
      Date.now() +
        this.parseExpiry(this.config.get<string>('JWT_REFRESH_EXPIRES_IN', '7d')),
    );
