...
describe('RolesService', () => {
  ...
  describe('update()', () => {
    ...
    it('throws BadRequestException for system roles', async () => {
      ...
      await expect(service.update('role-1', { name: 'Hack' }, 'org-1', 'actor-1')).rejects.toThrow(
        new BadRequestException('Cannot modify system roles')
      )
    })
  })
  ...
  describe('assignRole()', () => {
    it('creates user-role assignment and emits role.assigned', async () => {
      ...
      await service.assignRole({ userId: 'user-1', roleId: 'role-1' }, 'org-1', 'actor-1')
