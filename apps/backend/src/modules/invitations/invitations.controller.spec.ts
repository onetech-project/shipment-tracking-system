...
describe('InvitationsController (integration)', () => {
  ...
  describe('DELETE /invitations/:id', () => {
    it('returns 204 on cancel', async () => {
      ...
      expect(mockService.cancel).toHaveBeenCalledWith(INVITATION_ID, USER_ID)
    })
  ...
  describe('POST /invitations/accept (public)', () => {
    it('returns 200 on successful accept', async () => {
      ...
      const res = await request(app.getHttpServer())
        ...
        .send({ token: 'valid-raw-token', username: 'newuser', password: 'password123' })
      expect(res.status).toBe(200)
      expect(res.body.status).toBe('accepted')
    })

    it('returns 400 on invalid or expired token', async () => {
      mockService.accept.mockRejectedValue(new BadRequestException('INVALID_OR_EXPIRED_INVITATION'))
      const res = await request(app.getHttpServer())
        .post('/invitations/accept')
        .send({ token: 'expired-token', username: 'newuser', password: 'password123' })
      expect(res.status).toBe(400)
    })
  })
})
