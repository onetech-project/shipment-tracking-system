...
describe('AirShipmentsService — runSyncCycle()', () => {
  ...
  beforeEach(async () => {
    sheetsService = {
      ...
      getConfigs: jest.fn().mockReturnValue([
        { sheetName: 'CompileAirCGK', tableName: 'air_shipments_cgk', uniqueKey: 'to_number', skipNullCols: true, headerRow: 1 },
        { sheetName: 'SUB', tableName: 'air_shipments_sub', uniqueKey: 'to_number', skipNullCols: true, headerRow: 1 },
        { sheetName: 'SDA', tableName: 'air_shipments_sda', uniqueKey: 'to_number', skipNullCols: true, headerRow: 1 },
        { sheetName: 'Data', tableName: 'rate_per_station', uniqueKey: ['origin_dc', 'destination_dc'], skipNullCols: true, headerRow: 1 },
        { sheetName: 'Master Data', tableName: 'route_master', uniqueKey: 'concat', skipNullCols: false, headerRow: 1 },
      ]),
    } as any;
  ...
  it('skips a sheet when uniqueKey column is missing from normalized headers', async () => {
    sheetsService.fetchAllSheets.mockResolvedValue([
      ...
      { sheetName: 'SUB', tableName: 'air_shipments_sub', uniqueKey: 'to_number', rows: [], headers: [] },
      { sheetName: 'SDA', tableName: 'air_shipments_sda', uniqueKey: 'to_number', rows: [], headers: [] },
      { sheetName: 'Data', tableName: 'rate_per_station', uniqueKey: ['origin_dc', 'destination_dc'], rows: [], headers: [] },
      { sheetName: 'Master Data', tableName: 'route_master', uniqueKey: 'concat', rows: [], headers: [] },
    ]);
  ...
  it('skips a row when no field has changed', async () => {
    ...
    // Pre-load "existing" row in the repository
    const cgkRepo = service['repoFor']('air_shipments_cgk') as jest.Mocked<Repository<AirShipmentCgk>>;
    (cgkRepo.find as jest.Mock).mockResolvedValue([
      { to_number: 'CGK-001', status: 'delivered', is_locked: null, last_synced_at: new Date() },
    ]);
