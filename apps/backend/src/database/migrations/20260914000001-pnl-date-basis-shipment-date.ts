...
// Adds `date` (the sheet's own TO date) as a fourth PnL date basis, and makes it the default.
//
// The three existing bases all record a movement event — when the TO was completed, when it landed
// at the destination WH, when it left origin — so a TO's P&L lands in whichever cycle that event
// fell in, not the cycle the shipment itself belongs to. `date` is the TO's own date, the one
// encoded in the TO number (TO20260526STXE2 -> 2026-05-26), so it is what the business reconciles
// against. It is also the only basis populated on every row: 66,203/66,203 parse, against 65,332
// for ata_vendor_wh_destination — the ~871 TOs the ata basis silently drops now have a cycle.
//
// Only the cycle column is new. The date half of the pair already exists: `shipment_date`, added by
// 20260808000001 for Barhal and indexed by idx_compile_shipment_date. It is a DATE rather than the
// TIMESTAMP the other three bases use, which every caller tolerates — pnl.service casts each date
// column with ::DATE anyway, and buildFilter's range comparison (>= $1::DATE, < $2::DATE + 1 day)
// is exact on a DATE. Verified against production: shipment_date equals
// pnl_parse_date(extra_fields->>'date')::date on all 66,203 rows, and every raw value is ISO at
// midnight, so nothing is lost by the narrower type.
//
// NOTE (carried forward from 20260830000001): v_pnl_to has no single definition — every migration
// retypes all ~270 lines, which is how the non-AWB exclusion was once lost without a git conflict.
// This body is 20260830000001's verbatim plus the two new columns; the compile-CTE AWB-shape WHERE
// is intact. apps/backend/src/modules/pnl/pnl-awb-exclusion.integration.spec.ts is the guard.
export class PnlDateBasisShipmentDate20260914000001 implements MigrationInterface {
  name = 'PnlDateBasisShipmentDate20260914000001'

  public async up(queryRunner: QueryRunner): Promise<void> {
    // Same tolerant parser the other three cycle columns use, over the sheet's `date` field.
    await queryRunner.query(`
      ALTER TABLE air_shipments_compileaircgk
        ADD COLUMN IF NOT EXISTS cycle_date TEXT
          GENERATED ALWAYS AS (pnl_cycle(extra_fields->>'date')) STORED
    `)
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE air_shipments_compileaircgk DROP COLUMN IF EXISTS cycle_date`
    )
  }

  // 20260830000001's index set, plus the new basis's cycle index when the columns are present.
  private async createIndexes(queryRunner: QueryRunner, withDateBasis: boolean): Promise<void> {
    ...
    if (withDateBasis) {
      await queryRunner.query(`CREATE INDEX idx_v_pnl_to_cycle_date ON v_pnl_to(cycle_date)`)
      await queryRunner.query(`CREATE INDEX idx_v_pnl_to_shipment_date ON v_pnl_to(shipment_date)`)
    }
  }

  private viewSql(withDateBasis: boolean): string {
    return withDateBasis ? this.viewWithDateBasis() : this.viewWithoutDateBasis()
  }

  // 20260830000001's filtered body plus c.cycle_date / c.shipment_date surfaced through both CTEs.
  private viewWithDateBasis(): string {
    return `
      ...
      compile AS (
        SELECT
          ...
          c.date_completed, c.date_ata, c.date_atd, c.cycle_date, c.shipment_date,
  ...
  // 20260830000001's filtered body verbatim — the state down() must restore.
  private viewWithoutDateBasis(): string {
    return `
      CREATE MATERIALIZED VIEW v_pnl_to AS
      WITH
      station_map AS (
