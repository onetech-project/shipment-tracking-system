...
// The Compile Air CGK sheet reuses the AWB column as a status marker. Cancelled shipments get
// "VOID" (any casing, sometimes with a trailing space), reweighed ones get "OVERWEIGHT" /
// "OVER WEIGHT", and a handful of rows carry the TO number by mistake. None of those are air
// waybills, but v_pnl_to treated them as ordinary AWB keys, which broke the P&L three ways:
//
//   1. awb_totals GROUP BY awb collapsed every "VOID" row in the dataset into ONE synthetic AWB,
//      so sum_gw_per_awb summed hundreds of unrelated shipments and each row's weight_share —
//      gross_weight / sum_gw_per_awb — came out near zero.
//   2. Those rows have no booking, so cost_total_awb is NULL and cost_to collapses to just the
//      SG Incoming part. Their revenue_total still landed in every revenue and margin total, so
//      the P&L showed revenue with no cost behind it.
//   3. COUNT(DISTINCT awb) counted "VOID" as one more shipment in the summary.
//
// A row with no AWB *yet* is a different case: the shipment is real, the number simply has not
// been filled in. Those stay — a real AWB or none at all, never a placeholder.
//
// Body is otherwise identical to 20260816000001-pnl-station-lookup, whose definition down()
// restores verbatim.
export class PnlExcludeNonAwbRows20260827000001 implements MigrationInterface {
  name = 'PnlExcludeNonAwbRows20260827000001'
  ...
  private viewSql(withAwbFilter: boolean): string {
    // A real AWB is digits and dashes: 11 digits as "126-93330366", occasionally with a house-part
    // suffix ("126-93258970-1"), and a few legacy 10-digit numbers. Whitespace — including the
    // trailing CR that survives the sheet import — is trimmed before the test, so a well-formed
    // number is never dropped over invisible characters.
    //
    // Deliberately a shape test, not a "VOID | OVERWEIGHT | ..." blacklist: the sheet's marker
    // vocabulary grows (VOID, OVERWEIGHT, OVER WEIGHT, a stray TO number so far) and a blacklist
    // silently readmits every future one. Anything that is not shaped like an AWB is not an AWB.
    //
    // NULL and blank pass: the shipment is real, the AWB just is not known yet.
    const awbFilter = withAwbFilter
      ? `
        WHERE c.awb IS NULL
           OR BTRIM(c.awb, E' \\t\\r\\n') = ''
           OR BTRIM(c.awb, E' \\t\\r\\n') ~ '^[0-9][0-9-]*$'`
    ...
    return `
      ...
      compile AS (
        ...
        LEFT JOIN station_map sm
         ...
         AND sm.destination_dc = BTRIM(c.extra_fields->>'destination')${awbFilter}
      ),
      awb_totals AS (
