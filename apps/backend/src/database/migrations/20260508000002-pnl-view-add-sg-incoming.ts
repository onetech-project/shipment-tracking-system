...
// Adds SG Incoming as a per-TO cost component looked up by
// (origin_station, dest_station). cost_sg_in_to = gross_weight * sg_inc.
// Total cost per TO = (AWB-level prorated cost) + per-TO SG In.
// COALESCE the SG In to 0 so a missing route doesn't NULL out the entire TO cost.
export class PnlViewAddSgIncoming20260508000002 implements MigrationInterface {
  name = 'PnlViewAddSgIncoming20260508000002'
