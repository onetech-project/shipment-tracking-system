...
// Fixes duplicate-`id` fan-out in v_pnl_to that broke `REFRESH MATERIALIZED VIEW CONCURRENTLY`
// (unique violation on idx_v_pnl_to_id).
//
// Root cause: the `booking` CTE selected every row of air_shipments_smu_rate_cgk_spx and was
// joined to the shipment rows on `awb` alone. That table's unique key was widened from [awb] to
// [awb, account, via, dest], so an awb can now legitimately carry more than one row (a clean rate
// row plus dirty/partial rows). Each shipment row (which supplies the view's `id`) then matched
// every booking row for its awb and was emitted N times.
//
// The shipment side (air_shipments_compileaircgk) only has `awb`, so it cannot disambiguate which
// rate row applies. We therefore collapse `booking` to exactly one row per awb, preferring the row
// whose account/via/dest are all populated (i.e. the row that can actually resolve a rate),
// tie-broken by most-recently-synced. Everything downstream is unchanged, so PnL numbers match the
// intended (clean-row) values while the refresh no longer collides.
//
// Body mirrors 20260615000002-pnl-expose-chwt (chwt_awb exposed); only the `booking` CTE changes.
// down() restores the previous (un-deduped) definition.
export class PnlDedupBookingPerAwb20260711000001 implements MigrationInterface {
  name = 'PnlDedupBookingPerAwb20260711000001'
  ...
  private viewSql(dedup: boolean): string {
    // One clean booking per awb (up), or the legacy un-deduped CTE (down).
    const bookingCte = dedup
      ? `
      booking AS (
