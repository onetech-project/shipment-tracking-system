  ...
  return (
    <Link
      ...
      className={cn(
        ...
        isActive ? 'bg-white/10 text-white' : 'text-sidebar-muted hover:bg-white/10 hover:text-white'
      )}
    >
  ...
  return (
    <nav data-sidebar className="flex h-full w-60 flex-col bg-sidebar px-3 py-4">
      <div className="mb-6 px-3">
        <h2 className="text-sm font-semibold uppercase tracking-widest text-sidebar-muted">Shipment Tracker</h2>
      </div>
      <div className="flex flex-1 flex-col gap-1 overflow-y-auto">
        ...
        <div className="mt-4">
          ...
          <div className="flex flex-col gap-1">
            <NavLink href="/shipments/upload" icon={<Upload size={16} />} label="Import PDF" onClick={onNavClick} />
            <NavLink href="/shipments/history" icon={<History size={16} />} label="Upload History" onClick={onNavClick} />
            <NavLink href="/shipments/scan" icon={<QrCode size={16} />} label="QR Scan" onClick={onNavClick} />
          </div>
        </div>
        {isAdminOrAbove && (
          <div className="mt-4">
            <p className="mb-1 px-3 text-[0.65rem] font-semibold uppercase tracking-widest text-sidebar-muted">Settings</p>
            <div className="flex flex-col gap-1">
              {isSuperAdmin && <NavLink href="/settings/organizations" icon={<Building2 size={16} />} label="Organizations" onClick={onNavClick} />}
              <NavLink href="/settings/roles" icon={<Shield size={16} />} label="Roles" onClick={onNavClick} />
              <NavLink href="/settings/users" icon={<Users size={16} />} label="Users" onClick={onNavClick} />
              <NavLink href="/settings/invitations" icon={<Mail size={16} />} label="Invitations" onClick={onNavClick} />
              {isSuperAdmin && <NavLink href="/settings/permissions" icon={<Key size={16} />} label="Permissions" onClick={onNavClick} />}
            </div>
          </div>
        )}
        {isSuperAdmin && (
          <div className="mt-4">
            <NavLink href="/audit" icon={<ClipboardList size={16} />} label="Audit Logs" onClick={onNavClick} />
          </div>
        )}
      </div>
