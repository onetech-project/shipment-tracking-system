...
export function useQrScanner(): UseQrScanner {
  ...
  const setPermissionFromError = useCallback((err: unknown) => {
  ...
  const requestPermission = useCallback(async (): Promise<boolean> => {
    ...
    // Probe via Permissions API first (no dialog)
    try {
      ...
      // Permissions API unsupported — fall through to getUserMedia probe
    }
    // State is 'prompt' — trigger the browser permission dialog with a minimal stream
    try {
      ...
      setPermissionFromError(err)
      return false
    }
  }, [setPermissionFromError])
  ...
  const startScanner = useCallback(async () => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [stopScanner, requestPermission, setPermissionFromError])
